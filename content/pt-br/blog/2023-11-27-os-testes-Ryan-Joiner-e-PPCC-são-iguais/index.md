...
A Tabela <a href="#tab:tcriticos">1</a> apresenta os valores críticos para `\(5\%\)` de significância variando o tamanho amostral entre `\(3\)` e `\(10\)` (valores críticos de Ryan and Joiner (1976) foram arredondados na terceira casa decimal para ter precisão similar aos dados de Looney and Gulledge (1985)). Como é possível observar, os valores críticos são muito parecidos, sendo iguais em alguns valores de `\(n\)`.
...
<img src="{{< blogdown/postref >}}index_files/figure-html/diffplot-3.png" alt="Comparativo entre as diferenças dos críticos entre os testes de Ryan-Joiner e Looney-Gylledge." width="960" />
<p class="caption">
<span id="fig:diffplot"></span>Figure 3: Comparativo entre as diferenças dos críticos entre os testes de Ryan-Joiner e Looney-Gylledge.
</p>
...
Mais de `\(65\%\)` das diferenças são nulas para dados com `\(5\%\)` e `\(10\%\)`, indicando a similaridade entre os testes testes nestes níveis de singinificância. Já para `\(1\%\)` temos que `\(67\%\)` das diferenças são ***positivas***, indicando que neste caso os valores críticos do teste de Ryan and Joiner (1976) são ***maiores*** do que os valores críticos Looney and Gulledge (1985).

## Conclusão

Nesta análise comparamos os testes de Ryan and Joiner (1976) Looney and Gulledge (1985). Vimos que a estatística de ambos os testes é estimada de forma idêntica. Depois, comparamos os valores críticos destes testes, e foi possível observar que eles são muito parecidos.
